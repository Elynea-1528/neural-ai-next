...
class EventBus(EventBusInterface, metaclass=SingletonMeta):
    ...
    async def start(self) -> None:
        ...
        self.logger.info("EventBus indítása...")
        ...
        try:
            ...
            self.logger.info("Publisher bind-olva", pub_url=pub_url)

            # Kis várakozás, hogy a bind teljesüljön
            await asyncio.sleep(0.1)

            self._running = True
            self.logger.info("EventBus elindítva")
        except Exception as e:
            self.logger.error("Nem sikerült elindítani az EventBus-t", error=str(e), exc_info=True)
    ...
    async def stop(self) -> None:
        ...
        self.logger.info("EventBus leállítása...")
        self._running = False
        ...
        self.logger.info("EventBus leállítva")
    ...
    async def publish(self, event_type: str, event: "BaseModel | list[BaseModel]") -> None:
        ...
        self.logger.debug("Esemény közzétéve", event_type=event_type, count=len(events_to_publish))
    ...
    def subscribe(self, event_type: str, callback: EventCallback) -> None:
        ...
        self.logger.info("Feliratkozás létrehozva", event_type=event_type)
    ...
    def unsubscribe(self, event_type: str, callback: EventCallback) -> None:
        ...
        if event_type in self._subscribers:
            if callback in self._subscribers[event_type]:
                ...
                self.logger.info("Leiratkozás", event_type=event_type)
    ...
    async def _dispatch_event(self, event_type: str, event_data: dict[str, Any]) -> None:
        ...
        try:
            ...
            self.logger.error("Hiba az esemény deserializálásakor", error=str(e), exc_info=True)
    ...
    def _deserialize_event(
        ...
        try:
            if event_type == "market_data":
                ...
                self.logger.warning("Ismeretlen eseménytípus", event_type=event_type)
                return None
        except Exception as e:
            self.logger.error("Hiba az esemény deserializálásakor", error=str(e), exc_info=True)
            return None
    ...
    async def run_forever(self) -> None:
        ...
        try:
            ...
            self.logger.error("Hiba a feliratkozáskor", error=str(e), exc_info=True)
            subscriber.close()
            raise

        self.logger.info("Subscriber csatlakozva", sub_url=sub_url)
        ...
        try:
            while self._running:
                try:
                    ...
                    self.logger.error("Hiba az esemény fogadásakor", error=str(e), exc_info=True)

        finally:
            subscriber.close()
            self.logger.info("Subscriber lezárva")
