...
class TestBootstrapIntegrationTestIntegration:
    ...
    def project_root(self) -> Path:
        ...
        # Assert
        assert root.exists()
        assert (root / "scripts" / "bootstrap_integration_test.py").exists()
        return root

    def test_bootstrap_integration_test_script_exists(self, project_root: Path) -> None:
        """Teszt: A bootstrap_integration_test.py script létezik.
        ...
        """
        # Arrange
        script_path = project_root / "scripts" / "bootstrap_integration_test.py"
    ...
    def test_bootstrap_integration_test_imports(self, project_root: Path) -> None:
        """Teszt: A bootstrap_integration_test modul importálható.
    ...
    def test_bootstrap_integration_test_has_main_function(self, project_root: Path) -> None:
        ...
        if str(project_root) not in sys.path:
            sys.path.insert(0, str(project_root))
        from scripts.bootstrap_integration_test import main

        # Act & Assert
        assert callable(main)

    def test_bootstrap_integration_test_execution_dry_run(self, project_root: Path) -> None:
        """Teszt: A bootstrap_integration_test.py script futtatható (dry run).
        ...
        result = subprocess.run(
            [python_executable, "-c", f"import sys; sys.path.insert(0, '{project_root}'); from scripts import bootstrap_integration_test"],  # noqa: E501
            cwd=project_root,
            capture_output=True,
            text=True,
            timeout=10
    ...
    def test_bootstrap_integration_test_has_shebang(self, project_root: Path) -> None:
        """Teszt: A bootstrap_integration_test.py script rendelkezik shebang-gel.
    ...
    def test_bootstrap_integration_test_has_docstring(self, project_root: Path) -> None:
        """Teszt: A bootstrap_integration_test modul rendelkezik docstring-gel.
        ...
        if str(project_root) not in sys.path:
            sys.path.insert(0, str(project_root))
        from scripts import bootstrap_integration_test

        # Act & Assert
        assert bootstrap_integration_test.__doc__ is not None
        assert len(bootstrap_integration_test.__doc__.strip()) > 0
