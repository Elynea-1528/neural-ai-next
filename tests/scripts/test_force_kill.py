...
class TestForceKillIntegration:
    ...
    def project_root(self) -> Path:
        ...
        # Assert
        assert root.exists()
        assert (root / "scripts" / "force_kill.py").exists()
        return root

    def test_force_kill_script_exists(self, project_root: Path) -> None:
        """Teszt: A force_kill.py script létezik.
        ...
        """
        # Arrange
        script_path = project_root / "scripts" / "force_kill.py"
    ...
    def test_force_kill_imports(self, project_root: Path) -> None:
        """Teszt: A force_kill modul importálható.
    ...
    def test_force_kill_execution_dry_run(self, project_root: Path) -> None:
        ...
        result = subprocess.run(
            [python_executable, "-c", f"import sys; sys.path.insert(0, '{project_root}'); from scripts import force_kill"],  # noqa: E501
            cwd=project_root,
            capture_output=True,
            text=True,
            timeout=10
    ...
    def test_force_kill_has_shebang(self, project_root: Path) -> None:
        """Teszt: A force_kill.py script rendelkezik shebang-gel.
    ...
    def test_force_kill_has_docstring(self, project_root: Path) -> None:
        """Teszt: A force_kill modul rendelkezik docstring-gel.
        ...
        if str(project_root) not in sys.path:
            sys.path.insert(0, str(project_root))
        from scripts import force_kill

        # Act & Assert
        assert force_kill.__doc__ is not None
        assert len(force_kill.__doc__.strip()) > 0
