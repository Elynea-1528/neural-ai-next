...
class TestCoreComponentsInterface:
    ...
    def test_interface_methods_are_abstract(self) -> None:
        ...
        for method_name in abstract_methods:
            ...
            if method_name in ["config", "logger", "storage"]:
                # Ezek property-k
                assert isinstance(method, property), f"{method_name} nem property"
                assert method.fget is not None, f"{method_name} property-nek nincs getter-e"
            else:
    ...
    def test_interface_has_correct_type_hints(self) -> None:
        ...
        assert inspect.signature(has_config_method).return_annotation is not (
            inspect.Signature.empty
        )
        assert inspect.signature(has_logger_method).return_annotation is not (
            inspect.Signature.empty
        )
        assert inspect.signature(has_storage_method).return_annotation is not (
            inspect.Signature.empty
        )
...
class TestCoreComponentFactoryInterface:
    ...
    def test_interface_methods_are_abstract_and_static(self) -> None:
        ...
        for method_name in required_methods:
            ...
            assert isinstance(
                inspect.getattr_static(CoreComponentFactoryInterface, method_name), staticmethod
            ), f"{method_name} nem statikus metódus"
    ...
    def test_factory_create_components_with_parameters(self) -> None:
        ...
        components = MockCoreComponentFactory.create_components(
            config_path="/path/to/config", log_path="/path/to/log", storage_path="/path/to/storage"
        )
